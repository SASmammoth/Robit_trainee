...
def main():
    ...
    print("도서 대출 관리 시스템 (명령어: add_book, borrow_book, return_book, status_book, user, list, Exit)")
    ...
    while True:
        ...
        if cmd == "Exit":
            ...
            if len(args) != 2:
                print("ERROR: 사용법 - add_book <책이름> <수량>")
                continue
            ...
            if len(args) != 2:
                print("ERROR: 사용법 - borrow_book <사용자이름> <책이름>")
                continue    
            system.borrow_book(args[0], args[1])
        elif cmd == "return_book":
            if len(args) != 2:
                print("ERROR: 사용법 - return_book <사용자이름> <책이름>")
                continue
            system.return_book(args[0], args[1])
        elif cmd == "status_book":
            if len(args) != 1:
                print("ERROR: 사용법 - status_book <책이름>")
                continue
