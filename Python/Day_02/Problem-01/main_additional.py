...
def students_list_cleaner(csv_reader):
    # 학생부의 reader를 받아와서 유효한 리스트를 반환함
    # additiohal에서 DicReader을 이용하기 위해 숫자 대신 키로 접근함
    clean_list = []
    for row in csv_reader:
        row["name"] = row["name"].strip()  # 이름과 점수 공백 제거
        row["score"] = row["score"].strip()
        try:
            int(row["score"])  # 예외를 이용하여 정수로의 변환을 시도함
        except ValueError as e:
            print(e, " :\n", row["name"], " : ", row["score"], " → 숫자 변환 실패")
            continue
        else:
            row["score"] = int(row["score"])

        if 0 <= row["score"] and row["score"] < 100:  # 성적의 유효성을 검토함
            clean_list.append(row)
        else:
            print(row["name"], " : ", row["score"], " → 허용 범위 초과")
    print(clean_list)
    return clean_list
...
def student_csv_opener(src_file_location, dist_file_location):
    ...
    try:
        ...
        with open(src_file_location, "r", newline="", encoding="utf-8") as file:
            reader = csv.DictReader(file) #reader -> DictReader
            clean_list = students_list_cleaner(reader)
    ...
    with open(dist_file_location, "w", newline="", encoding="utf-8") as file:
        writer = csv.DictWriter(file, fieldnames= ["name", "score"]) #writer -> DictWriter로 수정함

        writer.writerows(clean_list) # DicRead로 읽은거라 바로 DicWriter로 넣을수 있음
...
def make_summary(clean_list, dist_file_location):
    ...
    for student in clean_list: #반복문을 이용하여 인원수, 평균, 최고점을 구한다.
        count += 1
        sum_score += student["score"] #딕셔너리 형식이라 키 값으로 접근
...
def main():
    ...
    new_file_name = "./clean_students-addithonal.csv"
    new_json_filename = "./summary-additional.json"
